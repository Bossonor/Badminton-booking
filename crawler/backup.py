...
def login():
    ...
    time.sleep(5)
    #command = "//*/div[@class='cardP15']/h3[text()='{name}']"
    #command = command.format(name = gym_name)
    #browser.find_element(By.XPATH, command).click()

def grab_order(times, url, date, hour, field):
    time_command = "//*/div[@class='el-tabs__nav is-top']/div[text()='{Date}']"
    time_command = time_command.format(Date = date)
    field_command1 = "//*/div[@class='inner-seat-wrapper clearfix']/div[{Hour1}]/div["
    field_command1 = field_command1.format(Hour1 = hour[0])
    field_command2 = "//*/div[@class='inner-seat-wrapper clearfix']/div[{Hour2}]/div["
    field_command2 = field_command2.format(Hour2 = hour[1])
    count = 0
    while True:
        ...
        if now > times:
            ...
            while True:
                ...
                #print("time", t2 - t1)
                try:
                        correct_time = WebDriverWait(browser, 1).until(EC.presence_of_element_located((By.XPATH, time_command)))
                        correct_time.click()
                    #if browser.find_element(By.XPATH, time_command):
                        #browser.find_element(By.XPATH, time_command).click()
                        for index in field:
                            print(index)
                            exp_cmd1 = field_command1 + index + "]/div[@class='inner-seat bought-seat']"
                            exp_cmd2 = field_command2 + index + "]/div[@class='inner-seat bought-seat']"
                            print(browser.find_element(By.XPATH, exp_cmd1))
                            if browser.find_element(By.XPATH, exp_cmd1) or browser.find_element(By.XPATH, exp_cmd2):
                                print("1")
                                continue
                            else:
                                print("2")
                                field_cmd1 = field_command1 + index + "]/div[@class='inner-seat unselected-seat']"
                                field_cmd2 = field_command2 + index + "]/div[@class='inner-seat unselected-seat']"
                                browser.find_element(By.XPATH, field_cmd1).click()
                                browser.find_element(By.XPATH, field_cmd2).click()
                                count += 1
                                break
